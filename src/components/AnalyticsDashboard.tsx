...
const Badge = ({percentage}: { percentage: number}) => {
    ...
    const positiveClassname = 'bg-gree-900/25 text-green-400 ring-green-400/25'
    const neutralClassname = 'bg-zinc-900/25 text-xinc-400 ring-zinc-400/25'
    const negativeClassname = 'bg-red-900/25 text-red-400 ring-red-400/25'

    return (
        <span className={`inline-flex gap-1 items-center roudned-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${
            isPositive
            ? positiveClassname
            : isNeutral
            ? neutralClassname 
...
const AnalyticsDashboard = ({
    avgVisitorisPerDay, amtVisitorsToday, timeseriesPageviews, topCountries
    ...
    return <div className='flex flex-col gap-4'>
        ...
        <Card className='w-full'>
            ...
            <Card className='w-full'>
                ...
                <Badge percentage={(amtVisitorsToday / Number (avgVisitorisPerDay) -1) * 100 }/>
                <p className='text-3xl text-tremor-content-strong font-semibold '>
                {amtVisitorsToday}
                </p>
            </Card>
